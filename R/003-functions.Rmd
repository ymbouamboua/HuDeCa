...
run_slingshot_on_pca_plot_umap <- function(
  ...
  if (is.null(start_clust)) {
    ...
    if (verbose) cat(" → Root:", start_clust, "\n")
  }
  ...
  if (is.null(end_clusts)) {
    ...
    if (verbose) cat(" → Terminals:", paste(end_clusts, collapse = ", "), "\n")
  }
...
#'------------------------------------------------------------------------------
#'------------------------------------------------------------------------------
#' Residualized mean UMI plot per cell type
#'
#' Computes donor-level mean UMI counts per cell type, residualizes them
#' against developmental stage (e.g. PCW) using linear regression, and
#' visualizes the residualized values stratified by group (e.g. sex).
#'
#' Supports boxplots (default) or bar plots with standard error, overlays
#' donor-level points colored by developmental stage, and optionally performs
#' per–cell type statistical testing between groups.
#'
#' @param data A Seurat object or a data.frame containing cell-level metadata.
#' @param cell.col Character. Column name specifying cell type annotations.
#' @param group.col Character. Column name specifying grouping variable
#'   (e.g. sex).
#' @param stage.col Character. Column name specifying developmental stage
#'   (numeric or extractable numeric, e.g. "PCW7").
#' @param plot.type Character. Plot type: `"box"` (default) or `"bar"`.
#' @param theme Character. Plot theme name (default `"classic"`).
#' @param group.colors Named character vector of colors for `group.col`.
#' @param stage.colors Named character vector of colors for `stage.col`.
#' @param jitter.size Numeric. Size of jittered points (default 1).
#' @param alpha Numeric. Transparency of jittered points (default 0.8).
#' @param xlab Logical. Display x-axis label (default TRUE).
#' @param ylab Logical. Display y-axis label (default TRUE).
#' @param x.angle Numeric. Rotation angle of x-axis labels (default 45).
#' @param font.size Numeric. Base font size (default 10).
#' @param leg.pos Character or numeric. Legend position (default `"top"`).
#' @param add_stats Logical. Whether to perform Wilcoxon rank-sum tests between
#'   groups within each cell type and display significance labels (default TRUE).
#' @param ... Additional parameters passed to downstream theme elements.
#'
#' @details
#' Mean UMI counts are first computed per donor and cell type. These values
#' are then residualized by linear regression against numeric developmental
#' stage to remove stage-dependent effects. Residuals are shifted to be
#' non-negative for visualization.
#'
#' Statistical testing is performed using two-sided Wilcoxon rank-sum tests
#' between groups within each cell type. Significance is displayed as:
...
#'   stage.colors = custom_colors$PCW
#' )
#' }
#'
#' @export
...
residual_plot <- function(
  ...
  for (col in c(cell.col, group.col, stage.col)) {
    if (!col %in% colnames(meta)) stop(paste0("Column '", col, "' not found."))
  }
  
  # Convert stage to numeric for residuals
  meta$stage_num <- suppressWarnings(as.numeric(gsub("[^0-9.]", "", meta[[stage.col]])))
  if (all(is.na(meta$stage_num))) stop("Stage column could not be converted to numeric.")
  
  # -----------------------------
  # Compute residualized mean UMIs per donor/cell type
  # -----------------------------
  donor_celltype <- meta %>%
    group_by(sample, .data[[cell.col]], .data[[group.col]], stage = .data[[stage.col]], stage_num) %>%
  ...
  # Merge residuals back into full metadata
  data_df <- meta %>%
    left_join(resid_df %>% select(sample, .data[[cell.col]], resid_mean_UMI_adj),
  ...
  # -----------------------------
  # Summary for bar plot / error bars
  # -----------------------------
  if (plot.type == "bar") {
    bar_df <- data_df %>%
      group_by(.data[[cell.col]], .data[[group.col]]) %>%
      summarise(
        mean_val = mean(.data[[value.col]], na.rm = TRUE),
  ...
  if (plot.type == "box") {
    ...
    p <- p + geom_bar(
      data = bar_df, aes_string(y = "mean_val"),
  ...
  # Stage points
  p <- p + geom_jitter(
    aes_string(color = stage.col),
    size = jitter.size, alpha = alpha,
    position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.8)
  )
  
  # -----------------------------
  # Wilcoxon stats per cell type
  # -----------------------------
  if (add_stats) {
    stat_df <- data_df %>%
      group_by(.data[[cell.col]]) %>%
      summarise(
        p_val = tryCatch(
          wilcox.test(.data[[value.col]] ~ .data[[group.col]], exact = FALSE)$p.value,
          error = function(e) NA
        ),
        max_val = max(.data[[value.col]], na.rm = TRUE), # max per cell type
        .groups = "drop"
      ) %>%
    ...
    # Add stat labels above each box/bar
    p <- p + geom_text(
      data = stat_df,
      aes_string(x = cell.col, y = "max_val * 1.05", label = "label"),
      inherit.aes = FALSE,
      position = position_dodge(width = 0.8),
      size = 3
    )
  ...
  p <- p +
    labs(x = ifelse(xlab, "", NULL),
         y = ifelse(ylab, "Residualized mean UMIs", NULL),
         fill = group.col,
         color = stage.col) +
...
expression_heatmap <- function(
  ...
  ## MODE 2: CELLTYPE × SAMPLE HEATMAP
...
plot_expression_heatmap <- function(
  ...
  # Order based on levels in Seurat object (generalized)
  ...
  # Auto-generate color palettes if missing
...
#' @export
#' 
identify_temporal_genes <- function(seurat_obj,
                                    sds_obj = NULL,
                                    pt_matrix = NULL,
  ...
  cells_sel <- WhichCells(seurat_obj, idents = selected_celltypes)
  pt_full <- if(is.null(pt_matrix)) slingPseudotime(sds_obj) else pt_matrix
  common_cells <- intersect(rownames(pt_full), colnames(seurat_obj))
  cells_use <- intersect(common_cells, cells_sel)
  pt_full <- pt_full[cells_use,, drop=FALSE]
  
  expr_all <- as.matrix(GetAssayData(seurat_obj, assay=assay, slot=expr_slot))[,cells_use, drop = FALSE]
