...
sub <- subset_seurat(obj,  group.by = "ann2",  use.harmony = T,  
                     harmony.group = "sample", min.dist = 1, spread = 0.5,
                     idents = c("INP", "iOSN"))
...
res$plot
savefig(fig = res$plot, filename = file.path(outdir, "plots/fig6c_supp"), width = 8, height = 4)
write.table(res$plot@data, file = file.path(outdir, "data/data_fig6c.tsv"), sep = "\t", row.names = F, quote = F)
...
sub <- subset_seurat(obj,  group.by = "ann2",  use.harmony = T,  harmony.group = "sample", 
...
p
savefig(fig = p, filename = file.path(outdir, "supp_fig6j"), width = 6, height = 7)
write.table(p@data, file.path(outdir, "data/data_supp_fig6j.tsv"), sep = "\t", row.names = F, quote = F)
