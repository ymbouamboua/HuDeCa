...
gam_pvals <- summary(gam_model)$s.table %>%
  ...
  dplyr::mutate(Lineage = gsub("s\\(stage\\):Lineage", "", Lineage),
                Lineage = plyr::mapvalues(Lineage, names(lineage_map), lineage_map),
                fdr = p.adjust(`p-value`, method = "fdr"))

# 4. Compute minimum FDR per stage for annotation
pw_pvals <- df %>%
  group_by(stage_label) %>%
  summarise(pval_min = min(gam_pvals$fdr[match(Lineage, gam_pvals$Lineage)]), .groups="drop") %>%
...
# 5. Compute peak density positions
peak_positions <- df %>%
  group_by(stage_label, Lineage) %>%
  summarise(dens = list(density(Pseudotime)), .groups = "drop") %>%
  rowwise() %>%
  mutate(
    y_max = max(dens$y),
  ...
  group_by(stage_label) %>%
  slice_max(y_max, n = 1) %>%  # highest peak per stage
  ungroup() %>%
  mutate(y_pos = y_max * 1.08)

# 6. Merge all into source_data
source_data <- df %>%
  left_join(gam_pvals %>% dplyr::select(Lineage, fdr), by = "Lineage") %>%
  left_join(peak_positions %>% dplyr::select(stage_label, Lineage, x_peak, y_pos),
            by = c("stage_label", "Lineage")) %>%
  left_join(pw_pvals %>% dplyr::select(stage_label, pval_label), by = "stage_label")
