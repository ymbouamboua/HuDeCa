...
exclude_terms <- c("FOREBRAIN", "BRANCHING", "KIDNEY", "RENAL","EXTERNAL","DNA GEOMETRIC","SPECIFICATION OF ANIMAL","GLAND DEVELOPMENT","HAIR","LAMB","NUCLEAR","DE NOVO","CHAPERONE","NEPHRON","APPENDAGE","SPINAL","HEAT", "EYE","MULTI-ORGANISM", "VISUAL","FILOPODIUM","RETINAL","COLD-INDUCED", "PROTEIN LOCALIZATION","SPERM AXONEME","LIMB","DEMANNOSYLATION","CYTOSKELETON","NON-MOTILE","CARDIAC","PURINE","RIBONUCLEOSIDE","RIBONUCLEOTIDE","METAL ION","MAINTENANCE OF LOCATION","PYRUVATE","NUCLEOTIDE","NUCLEOSIDE","PYRIDINE","RIBONUCLEOSIDE","NEPHRON","ENDOSOME","MOTOR NEURON","FRUCTOSE","WOUND HEALING","TISSUE HOMEOSTASIS","FAT CELL","INTERNAL ENCAPSULATING","SKIN","BODY FLUID","CARBOHYDRATE","PREGNANCY","MECHANCAL","KETONE","PEPTIDE HORMONE","LUNG","RESPIRATORY TUBE","ENDODERM","CIRCADIAN","PLASMA MEMBRANE","WOUNDING")

res <- readRDS(file.path(dir.results, "BP/OE_PCW_progression_markers_GO_terms/PCW7_GO_BP.rds"))
p <- Plot_GO_Enrichment(df = res@result, category = 30,wrap_length = 50, base_size = 8,
                   exclude_terms = exclude_terms, cluster_name = "PCW7")
p
save_plot(p, filename = file.path(dir.results, "figures/FIg_S3a"),  width = 8, height = 7, formats = c("pdf"))

res <- readRDS(file.path(dir.results, "BP/OE_PCW_progression_markers_GO_terms/PCW8_GO_BP.rds"))
p <- Plot_GO_Enrichment(df = res@result, category = 30,wrap_length = 50, base_size = 8,
                   exclude_terms = exclude_terms, cluster_name = "PCW8")
p
save_plot(p, filename = file.path(dir.results, "figures/FIg_S3b"),  width = 8, height = 7.5, formats = c("pdf"))

res <- readRDS(file.path(dir.results, "BP/OE_PCW_progression_markers_GO_terms/PCW10_GO_BP.rds"))
p <- Plot_GO_Enrichment(df = res@result, category = 30,wrap_length = 60, base_size = 8,
                   exclude_terms = exclude_terms, cluster_name = "PCW10")
p
save_plot(p, filename = file.path(dir.results, "figures/FIg_S3c"),  width = 8, height = 7, formats = c("pdf"))

res <- readRDS(file.path(dir.results, "BP/OE_PCW_progression_markers_GO_terms/PCW12_GO_BP.rds"))
p <- Plot_GO_Enrichment(df = res@result, category = 30,wrap_length = 50, base_size = 8,
                   exclude_terms = exclude_terms, cluster_name = "PCW12")
p
save_plot(p, filename = file.path(dir.results, "figures/FIg_S3d"),  width = 8, height = 7, formats = c("pdf"))
...
```{r fig.width=4, fig.height=3.5, warning=F,message=F}

pdf(file = file.path(dir.results, "figures/Fig_S5a.1.pdf"),  width = 4, height = 3.5)
Plot_Cluster_Correlation(obj, group.by = "ann_level_0", fontsize = 10, plot.title = "", column_names_rot = 90)
dev.off()

```

```{r fig.width=7.5, fig.height=6.5, warning=F,message=F}

pdf(file = file.path(dir.results, "figures/Fig_S5a.2.pdf"),  width = 7.5, height = 6.5)
Plot_Cluster_Correlation(obj, group.by = "ann_level_2", fontsize = 10, plot.title = "", column_names_rot = 90)
dev.off()
...
```{r fig.width=12, fig.height=7, warning=FALSE, message=F}

Idents(obj) <- "ann_level_1"
sub <- subset(obj, idents = "Olfactory epithelium", invert = T)
p <- plot_cluster_percent(data = sub, label_size = 3, cluster_col = "ann_level_2_order", group_col = "PCW_order",sample_col = "sample", 
group_colors = custom_colors$PCW,legend = FALSE, multi_p = F,ncol = 7,x_angle = 45)
p
save_plot(p, file.path(dir.results, "figures/Fig_S5b"), formats = c("pdf"),width = 12, height = 7)
...
obj <- readRDS(file.path(dir.main, "results/003_celltype/rdata/FN.rds"))
celltypes <- c("Olf. HBCs","Microvillars","Sustentaculars", "GBCs","INPs","iOSNs","mOSNs","Olf. ensh. glia")
colors <- c("Olf. HBCs"="#E41A1C","GBCs"="#F48B5A","INPs"= "#E69F00","iOSNs"="#f05b43","mOSNs"="#33b8ff", "Microvillars"="#efe13c",
"Sustentaculars"="#C09ACA","Olf. ensh. glia" = "palevioletred3")

Idents(obj) <- "ann_level_2"
sub <- Seurat_Subset(obj, cluster_id = celltypes, ndims = 5, min.dist = 1)

# Create cellchat object
cellchat <- createCellChat(object = sub, group.by = "ann_level_2_order", assay = "RNA")
rm(sub)
invisible(gc())
...
# Filter out the cell-cell communication if there are only few cells in certain cell groups. By default, the minimum number of cells required in each cell group for cell-cell communication is 10.
...
saveRDS(cellchat, file.path(dir.results, "rdata/FN_cellchat.rds"))
invisible(gc())
...
ht1 <- netAnalysis_signalingRole_heatmap(cellchat, pattern = "outgoing", font.size = 12, width = 6, height = 8, color.heatmap = "BuGn",  cluster.rows = F, cluster.cols = TRUE, color.use = colors)

ht2 <- netAnalysis_signalingRole_heatmap(cellchat, pattern = "incoming", font.size = 12, width = 6, height = 8, color.heatmap = "BuGn",  cluster.rows = F, cluster.cols = TRUE, color.use = colors)

ComplexHeatmap::draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))

pdf(file.path(dir.results, "figures/Fig_S7a.pdf"), width = 7.5, height = 6, useDingbats = F)
...
dir <- file.path(dir.results, "figures/SLIT_network/")
if (!dir.exists(dir)) {
  dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)
}

LR.pairs <- read.delim(file.path(dir.results, "figures/LR.pairs/LR.pairs.tsv"))
pathway.name <- unique(LR.pairs$pathway_name)
ann_level_2.to.test <- as.vector(unique(cellchat@idents))
ann_level_2.to.test <- ann_level_2.to.test[ann_level_2.to.test %in% c("preOSNs","iOSNs","mOSNs","Olf. ensh. glia")]

pathways.show = "SLIT"
weight.max <- max(cellchat@netP$weights[[pathways.show]], na.rm = TRUE)

pdf(file = file.path(dir, paste0(pathways.show, "_Fig_S7b.pdf")), width = 8, height = 8, useDingbats = FALSE)
...
netVisual_aggregate(
  ...
  pt.title = 14,
  layout = "chord",
  sources.use = ann_level_2.to.test,  
  targets.use = ann_level_2.to.test,  
  color.use = colors, 
  cell.order = celltypes,  
...
invisible(gc())
dir <- paste(dir.results, "figures/pathway_signaling_roles/", sep = "/")
if(!dir.exists(dir)){dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)}

#pathways.show <- cellchat@netP$pathways
pathways.show <- c("PTN","ncWNT","SEMA3","FGF","IGF","SLIT")

for(i in 1:length(pathways.show)){
  print(pathways.show[i])
  pdf(paste(dir,  pathways.show[i], ".pdf", sep = ""), width = 6, height = 2.5, useDingbats = FALSE)
  netAnalysis_signalingRole_network(cellchat_subset, signaling = pathways.show[i],  width = 6,
                                    height = 2.5, font.size = 10, color.use = colors) 
  dev.off()
}
...
## Extanded data figures 8

```{r  fig.width=20, fig.height=12, warning=F, message=F}

# Set output directory
dir <- file.path(dir.results, "figures/LR.pairs.all.pathways/")
if (!dir.exists(dir)) {
  dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)
}

# Generate a single bubble plot for all pathways
netVisual_all <- netVisual_bubble(cellchat,
                                  remove.isolate = TRUE,
                                  sources.use = ann_level_2.to.test,
                                  targets.use = ann_level_2.to.test,
                                  angle.x = 45,
...
pdf(file = paste0(dir, "All_Pathways_LR_pairs.pdf"), width = 18, height = 12, useDingbats = FALSE)
print(netVisual_all)
dev.off()

print("Unique plot for all pathways across all cell types saved successfully.")


```

## Extanded data figures 9
...
colors <- c("Olf. HBCs"="#E41A1C","GBCs"="#F48B5A","INPs"= "#E69F00","iOSNs"="#f05b43","mOSNs"="#33b8ff", "Microvillars"="#efe13c","Sustentaculars"="#C09ACA","Olf. ensh. glia" = "palevioletred3")


# Define the directory for saving signaling network plots
dir <- file.path(dir.results, "figures/signaling_network/")
if (!dir.exists(dir)) {
  dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)
}
...
#pathway.name <- unique(LR.pairs$pathway_name)
pathway.name <- c("PTN","ncWNT","SEMA3","FGF","IGF","SLIT")

ann_level_2.to.test <- as.vector(unique(cellchat@idents))
ann_level_2.to.test <- ann_level_2.to.test[!ann_level_2.to.test %in% c("Schwann precursors","Schwann cells","Neural crest", "Melanocytes")]

# Loop through pathways
for (pathway in pathway.name) {
  
  # Show the current pathway being processed
  pathways.show <- pathway
  
  # Get the maximum weight for the current pathway
  weight.max <- max(cellchat@netP$weights[[pathways.show]], na.rm = TRUE)
  
  # Generate and save the plot for the single CellChat object
  pdf(
    file = file.path(dir, paste0(pathways.show, "_Fig_S8.pdf")),
    width = 8, 
    height = 8, 
    useDingbats = FALSE
  )
  
  # Visualize the signaling network
  netVisual_aggregate(
    cellchat,
...
dir <- file.path(dir.results, "figures/LR.pair_contributions/")
if (!dir.exists(dir)) {
  dir.create(file.path(dir), recursive = TRUE, showWarnings = FALSE)
}

# Load the list of ligand-receptor pairs
LR.pairs <- read.delim(file.path(dir.results, "figures/LR.pairs/LR.pairs.tsv"))

# Extract the unique pathway names
pathway.name <- c("PTN","ncWNT","SEMA3","FGF","IGF","SLIT")
...
for (pathway in pathway.name) {
  ...
  # Generate the net analysis contribution plot for the single CellChat object
  ...
  # Save the plot
  save_plot(
    file.path(dir, paste0(pathways.show, "_CellChat")),
    plot = p,           
    width = 3.5,
    height = 5,
    formats = c("pdf", "png")
...
### Plot pathway gene expressions

# plotGeneExpression

```{r fig.width=3, fig.height=5, warning=F, message=F}

dir <- file.path(dir.results, "figures/plotGeneExpression/")
...
for (pathway in 1:length(pathway.name)) {
  ...
  genes <- toupper(genes)
  genes <- rownames(sub[genes,])

  p <- Stacked_VlnPlot(object = sub, 
               x_angle = 45,x_vjust = 1, x_hjust = 1,
               features = genes,
               group.by = "ann_level_2_order", 
               font_size = 10,
               colors.use = colors,
               color.text.x = NULL)
  
   save_plot(p, paste0(dir, pathways.show,"_gene_expression"),  width = 3, height = 4)
