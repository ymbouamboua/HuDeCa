...
p
savefig(fig = p, filename = file.path(outdir, "plots/figS3d_left"), width = 3, height = 3)
write.table(p1@data, file.path(outdir, "data/data_source_figS3d_left.tsv"), sep = "\t", row.names = F, quote = F)
...
p <- ggplot(tbl_summary, aes(x = sample, y = median_eryth_sum)) +
...
p
savefig(fig = p, filename = file.path(outdir, "plots/figS3e"), width = 3, height = 3)
write.table(p2@data, file.path(outdir, "data/data_souirce_figS3d_right.tsv"), sep = "\t", row.names = F, quote = F)
